...
/// Routes the provided `McpCli` command to the appropriate execution handler based on its variant.
pub async fn execute_cli_router(cmd: McpCli) -> Result<()> {
    match cmd {
        McpCli::Client { conf, action } => {
            let router = ClientCli::new(conf, action);
...
impl ClientCli {
    pub fn new(conf: String, cmd: ClientCommands) -> Self {
        Self { conf, cmd }
    }
    /// Formats the given serializable data into either a raw JSON string or a colored hierarchical tree.
...
impl CliExecute for ClientCli {
    async fn execute(&self) -> Result<()> {
        let client = ClientEngine::connect(&self.conf).await?;
