...
pub async fn run_interactive_router() -> Result<()> {
    ...
    println!(
        "Welcome to pgmoneta MCP Client Interactive Shell! v{}",
        client_version
    );
...
impl InteractiveWizard for ClientWizard {
    async fn run(&self) -> Result<()> {
        let default_conf = "/etc/pgmoneta-mcp/pgmoneta-mcp-client.conf";
        ...
        let client = ClientEngine::connect(&conf_path).await?;

        if let Some((server_name, server_version, url)) = client.server_info() {
        ...
        loop {
            ...
            match choice {
                Ok("Tools") => match PageTool::client_command(&client).await {
                    Ok(Some(cmd)) => {
                        let conf_to_pass = conf_path.clone();
                        let router = ClientCli::new(conf_to_pass, cmd);
